...
t_args* spawn_threads(pthread_t *thread_handles,int num_threads,int from,int to,float *mVec,float *nVec,float *RVec,float *LVec,float *CVec,float *FVec);
...
int main(int argc, char ** argv) {
  ...
  if(proc_id==0){
    int start_index[proc_count];
    int end_index[proc_count];
    for(unsigned int i=0;i<proc_count;i++){
      start_index[i] = i*(N/proc_count);
      end_index[i] = (i*(N/proc_count))+(N/proc_count);
    }
    for(unsigned int i=1;i<proc_count;i++){
      MPI_Send(&start_index[i], 1, MPI_INT, i, 1, MPI_COMM_WORLD);
      MPI_Send(&end_index[i], 1, MPI_INT, i, 2, MPI_COMM_WORLD);
      main_args = spawn_threads(thread_handles,num_threads,0,N/proc_count,mVec,nVec,RVec,LVec,CVec,FVec);
    }
  }
  else{
    int from,to;
    MPI_Recv(&from, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, &status);
		MPI_Recv(&to, 1, MPI_INT, 0, 2, MPI_COMM_WORLD, &status);
    if(proc_id == proc_count-1){
      main_args = spawn_threads(thread_handles,num_threads,from,N,mVec,nVec,RVec,LVec,CVec,FVec);
    }
    else{
      main_args = spawn_threads(thread_handles,num_threads,from,to,mVec,nVec,RVec,LVec,CVec,FVec);
    }
  }
  ...
  if(proc_id==0){
    float maxF_array[proc_count];
    float minF_array[proc_count];
    float avgF_array[proc_count];
    ...
    for(unsigned int i=1;i<proc_count;i++)
		{
			MPI_Recv(&maxF_array[i], 1, MPI_INT, i, 3, MPI_COMM_WORLD, &status);
			MPI_Recv(&minF_array[i], 1, MPI_INT, i, 4, MPI_COMM_WORLD, &status);
			MPI_Recv(&avgF_array[i], 1, MPI_INT, i, 5, MPI_COMM_WORLD, &status);
		}
    avgF = 0.0f;
    for(unsigned int i=0;i<proc_count;i++){
      if(minF_array[i]<minF){minF = minF_array[i];}
      if(maxF_array[i]>maxF){maxF = maxF_array[i];}
      avgF += avgF_array[i];
    }

    double timeOmegaTotal = gettime() - timeOmegaTotalStart;
    double timeTotalMainStop = gettime();
    printf("Omega time %fs - Total time %fs - Min %e - Max %e - Avg %e\n",timeOmegaTotal, timeTotalMainStop - timeTotalMainStart, (double) minF, (double) maxF,  (double) avgF / N);
  }
  else{
    process_workload(&avgF,&minF,&maxF,num_threads,thread_handles,main_args);
    MPI_Send(&maxF, 1, MPI_FLOAT, 0, 3, MPI_COMM_WORLD);
		MPI_Send(&minF, 1, MPI_FLOAT, 0, 4, MPI_COMM_WORLD);
		MPI_Send(&avgF, 1, MPI_FLOAT, 0, 5, MPI_COMM_WORLD);
  }
...
t_args* spawn_threads(pthread_t *thread_handles,int num_threads,int from,int to,float *mVec,float *nVec,float *RVec,float *LVec,float *CVec,float *FVec){
  ...
  int range_array[num_threads+1];
  int nt;
  int c = (float)(to-from)/((float)num_threads+1);
  c = c -(c%4);
  for(unsigned int t=0; t<=num_threads;t++){
    ...
    args->FVec = FVec;
    if(t == 0){
      args->from_index = from;
      args->to_index = from + c;
      count += c;
      main_args = args;
    }
    else
    {
      if(t<num_threads){
        args->from_index = from+count;
        args->to_index = from+count+c;
        count+= c;
      }
      else{
        args->from_index = from+count;
    ...
    if(t!=0){pthread_create(&thread_handles[t-1],NULL,omega_calc,(void*)args);}
  }
  return main_args;
}
...
void *omega_calc(void * args){
  ...
  for(unsigned int i=((t_args*)args)->from_index;i<t;i+=4){
  ...
  t = (((t_args*)args)->from_index - ((t_args*)args)->to_index)-(((t_args*)args)->from_index - ((t_args*)args)->to_index%4);
